...
if (isset($_POST['Submit'])) {
	...
	if($leaveType == "Personal" && empty($personalReason)) {
		...
		$leave0_sql = "SELECT * FROM leave_application WHERE id_user = '$userID' AND leave_type = '$leaveType' AND start_date = '$startDate' AND end_date = '$endDate' and id != $'IDleave' ";
		...
		alert('leave application existed');
			...
			$leave_sql = "UPDATE leave_application SET leave_type = '$leaveType', personal_reason = '$personalReason', start_date = '$startDate', end_date = '$endDate' WHERE id = '$IDleave'";
			...
			<div class="col-lg-12">	
				<div class="leave-applications" data-aos="fade-down">
					<div class="item">
						...
						<form method="POST" action="user.php?userpage=editLeaveApplication&ID=<?=$leave1['id'];?>"  class="form beta-form-checkout">
						...
						<div class="form-group select">
				    		<label for="leave_type">Leave Reason:</label>
					   		<select class="form-control" name="leave_type" required="">
						   		...
						   		<option value="Personal" <?php if($leave1['leave_type'] == "Vacation") { ?> selected="selected"  <?php } ?> >Personal</option>
						      	<option value="Sick Leave" <?php if($leave1['leave_type'] == "Vacation") { ?> selected="selected"  <?php } ?> >Sick Leave</option>
							</select>
						</div>
						...
						<div class="form-group">
							...
							<input type="date" name="start_date" class="form-control" value="<?= $leave1['start_date']; ?>" required="">
						</div>

						<div class="form-group">
							<label for="date">End Date:</label>
							<input type="date" name="end_date" class="form-control" value="<?= $leave1['end_date']; ?>" required="">
						</div>
